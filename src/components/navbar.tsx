...
export default function Navbar(params: { locale: string | undefined }) {
    const {locale} = params
    ...
    return (
        <div
            ...
            <Dock
                ...
                <Separator orientation="vertical" className="h-full py-2"/>
                <DockIcon key={"langSwitch"}>
                    <Tooltip>
                        <TooltipTrigger asChild>
                            <Link
                                href={(locale || routing.defaultLocale) == routing.defaultLocale ? "/en-us" : "/"}
                                className={cn(
                                    buttonVariants({variant: "ghost", size: "icon"}),
                                    "size-12"
                                )}
                            >
                                <Icons.multilingual className={"size-4"}/>
                            </Link>
                        </TooltipTrigger>
                    </Tooltip>
                </DockIcon>
